...
rel_day = "(today|yesterday|tomorrow|tonight|tonite|till|until|since)"
exp1 = "(before|after|earlier|later|ago)"
exp2 = "(this|next|last|upcoming|following|until|since|till)"
iso = "\d+[/-]\d+[/-]\d+ \d+:\d+:\d+\.\d+"
year = "((?<=\s)\d{4}|^\d{4})"
...
def ground(tagged_text, base_date):
    ...
    for timex in timex_found:
        ...
        elif re.search(r'this ' + month, timex, re.IGNORECASE):
            month = hashmonths[timex.split()[1]]
